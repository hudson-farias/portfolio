...
function FrameworkTile({ framework }: { framework: Framework }) {
  ...
  const scope = framework.scope === "backend" ? "Backend" : framework.scope === "frontend" ? "Frontend" : null
...
export function StackTech({
  ...
  const scopeOptions = [
    ...(hasBackend ? [{ value: "backend" as const, label: "Backend" }] : []),
    ...(hasFrontend ? [{ value: "frontend" as const, label: "Frontend" }] : []),
  ]

  if (frameworks.length === 0) return null
  if (!hasBackend && !hasFrontend) return null

  const emptyLabel = `Nenhum framework em ${effectiveScope === "backend" ? "backend" : "frontend"}.`

  return (
    <section id="stack-tech" className="relative isolate z-0 scroll-mt-28 space-y-10">
      <Reveal className="mx-auto max-w-2xl space-y-3 text-center">
        <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Linguagens e frameworks</h2>
        <p className="text-muted-foreground">
          O que uso para construir backend, frontend e automações.
        </p>
      </Reveal>
      ...
      {items.length > 0 ? (
        <Reveal delay={160}>
          <div className="flex justify-center">
            ...
            >
              {hasMore ? "Ver todos os frameworks" : "Ver frameworks"}
