...
export function StackDatabases({
  ...
  const scopeOptions = [
    ...(hasSql ? [{ value: "sql" as const, label: "SQL" }] : []),
    ...(hasNosql ? [{ value: "nosql" as const, label: "NoSQL" }] : []),
  ]

  if (databases.length === 0) return null
  if (!hasSql && !hasNosql) return null

  const emptyLabel = `Nenhum banco em ${effectiveScope === "sql" ? "SQL" : "NoSQL"}.`

  return (
    <section id="stack-databases" className="relative isolate z-0 scroll-mt-28 space-y-10">
      <Reveal className="mx-auto max-w-2xl space-y-3 text-center">
        <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Banco de dados</h2>
        <p className="text-muted-foreground">
          Tecnologias de persistência que uso em projetos backend e full stack.
        </p>
      </Reveal>
      ...
      {items.length > 0 ? (
        <Reveal delay={160}>
          <div className="flex justify-center">
            ...
            >
              {hasMore ? "Ver todos os bancos" : "Ver bancos de dados"}
